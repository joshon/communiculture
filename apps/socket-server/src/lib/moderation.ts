...
export function initModerationServer(io: Server) {
  ioRef = io;
}
...
async function processBatch(items: QueueItem[]) {
  ...
  const response = await getClient().messages.create({
    ...
    messages: [{ role: "user", content: `Moderate these chat messages:\n${itemsJson}` }],
  });
...
async function applyResults(items: QueueItem[], results: ModerationResult[]) {
  for (const result of results) {
    ...
    console.warn(`[moderation] rejected message:${item.messageId} — ${result.reason}`);
    ...
    try {
      ...
      // Retract the message from all clients in the room
      if (ioRef) {
        ioRef.to(item.continuumId).emit("chat:retract", { messageId: item.messageId });
      }
    } catch (err) {
      console.error(`[moderation] failed to retract message ${item.messageId}:`, err);
    }
  }
}
